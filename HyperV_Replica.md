...
This document describes how to maintain Hyper-V Replica after a failover.

---- 

1.	Prepare 3 servers with Windows Server 2019 (2 x for ECX, 1 for the replication server)
...
4.	Follow directions from the [Quick Start Guide for Windows Server 2016 Hyper-V](https://www.nec.com/en/global/prod/expresscluster/en/doc/guide.html?#:~:text=May%2010%2C%202017-,Virtualization,-Hyper%2DV) to set up Hyper-V, install and configure ExpressCluster X, and create a VM on the mirror disk on Server 1 and Server 2. The inline documentation has been moved to a [\.PDF file](https://www.nec.com/en/global/prod/expresscluster/en/support/setup/ECXW_WS2016Hyper-V_QSG_01.pdf). Jump to Chapter 4 **Hyper-V Setup**.

	> Although the guide referenced above was created for Windows Server 2016, the installation and configuration steps should be similar to Windows Server 2019.

	**Note**: If the Primary and Standby ECX servers are VMs (assuming nested virtualization), run the following commands on the host machine for each ECX VM to allow the installation of Hyper-V. The ECX VMs must be turned off in order to run these commands:
...
5.	Download and extract [ECX Tools](https://github.com/EXPRESSCLUSTER/Tools/archive/master.zip) on both servers and run **Openports.bat** to allow ExpressCluster to communicate through the firewall
...
9.	On the Hyper-V Replica server, open Hyper-V Manager, right click on the VM, and **Remove Replication**.
	* Note: Do **NOT** remove the VM from Hyper-V manager or make any other modifications to the VM files on this server. The VM copy will be used as the source for initializing replication from the Secondary server and it will not be necessary to recopy the large virtual hard disk files.
	...
	Alternatively, the same operation can be run on the failover destination server (Server 2 in this example) with the PowerShell command below.

		PS C:\> Remove-VMReplication -ComputerName <Replica Server> -VMName <VM Name>

	Here, "Replica Server" is Server 3. "VM Name" is the VM to be replicated.
...
10.	Re-enable replication on the Secondary server
	...
	* Note: The VM will continue to run normally. There is a synchronization process that runs in the background between the VM files on the Secondary server and the Replica server VM files, merging any changes. This process can take a number of minutes.    
	
	    Steps 8 - 10 can be run again to move the VM back to the Primary server.
